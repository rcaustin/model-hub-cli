...
class PerformanceClaimsMetric(Metric):
    ...
    Returns a score from 0.0 (no claims or misleading) to 1.0 (clear, quantified claims).
    """
    ...
    def evaluate(self, model: ModelData) -> float:
        ...
        logger.info("PerformanceClaimsMetric: {} claims found, average score {} -> {}", 
    ...
    def _extract_hf_claims(self, model: ModelData) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(tags, list):
                for tag in tags:
                    if isinstance(tag, str) and any(pattern in tag.lower() for pattern in ['accuracy', 'f1', 'bleu', 'rouge']):
                        claims.append({
                            'text': tag,
                            'source': 'hf_tags',
                            'context': 'model tags'
    ...
    def _score_claim(self, claim: Dict[str, Any]) -> float:
        ...
        is_performance_metric = any(metric in text for metric in 
                                  ['accuracy', 'f1', 'precision', 'recall', 'auc', 'bleu', 'rouge', 'perplexity'])
